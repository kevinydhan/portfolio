...
const IndexPage = (): JSX.Element => {
  ...
  return (
    <>
      <Head />
      <div className={`${sharedStyles.grid} ${sharedStyles.backgroundGrid}`}>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
      ...
      <main className={sharedStyles.grid}>
        ...
        <section id="projects" className={sharedStyles.gridChild}>
          <h2 className={sharedStyles.projectSectionHeading}>Latest Works</h2>
          {projects.map((project) => (
            <ProjectCard key={project.imgDetails.originalName} {...project} />
          ))}
        </section>
      </main>
    </>
